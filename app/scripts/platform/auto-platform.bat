...
powershell -Command "try { Invoke-WebRequest -Uri 'http://%PLATFORM_NAME%.io' -TimeoutSec 3 -UseBasicParsing | Out-Null; Write-Host '✅ Domain ready: http://%PLATFORM_NAME%.io' -ForegroundColor Green } catch { Write-Host '💡 Domain: Run scripts\setup-domains.bat for domain access' -ForegroundColor Cyan }" 2>nul
